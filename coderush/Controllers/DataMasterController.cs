    ...
    public class DataMasterController : Controller
    {
        ...
        public ActionResult Index(string Searcvalue)
        {
            ...
            else
            {
                ...
                else if(Searcvalue == "4")
                {
                    ViewBag.key = "4";
                    datamaster = _context.Datamaster.Where(x => !x.Isdeleted && x.Type == DataSelection.HRAnnocuncement).ToList();
                }
                else if (Searcvalue == "15")
                {
                    ViewBag.key = "15";
                    datamaster = _context.Datamaster.Where(x => !x.Isdeleted && x.Type == DataSelection.EmployeeType).ToList();
                }
                else if (Searcvalue == "16")
                {
                    ViewBag.key = "16";
                    datamaster = _context.Datamaster.Where(x => !x.Isdeleted && x.Type == DataSelection.LeaveReasons).ToList();
                }
            }
