...
namespace coderush.Controllers
{
    ...
    public class ExpenseMasterController : Controller
    {
        ...
        public IActionResult ExpenseIndex(string sdate, string edate, string curentmonth, string lastmont)
        {
            ...
            var monthStart = new DateTime(todaynow.Year, todaynow.Month, 1);
            var monthEnd = monthStart.AddMonths(1).AddDays(-1);
            var lastMonthStart = monthStart.AddMonths(-1);
            var lastMonthEnd = monthStart.AddDays(-1);

            var lastmonthexpense = serchadata.List.Where(x => x.Isdelete == false
                                         && x.ExpenseDate >= Convert.ToDateTime(lastMonthStart) && x.ExpenseDate <= Convert.ToDateTime(lastMonthEnd)).Sum(x => x.Amount);

            var lastmonthcredit = _context.Credit.Where(x => x.Creditdate >= Convert.ToDateTime(lastMonthStart) && x.Creditdate <= Convert.ToDateTime(lastMonthEnd)).Sum(x => x.Amount);
            ViewBag.LastMonthBalance = lastmonthcredit - lastmonthexpense;

            serchadata.CreditList = (from credit in _context.Credit
                                     where credit.Creditdate >= (sdate != null && sdate != "" ? Convert.ToDateTime(sdate) : Convert.ToDateTime(monthStart))
                                     && credit.Creditdate <= (edate != null && edate != "" ? Convert.ToDateTime(edate) : Convert.ToDateTime(monthEnd))
                                     orderby credit.Id descending
                                     select new CreditMasterViewModel
                                     {
                                         Id = credit.Id,
                                         Managername = credit.Managername,
            ...
            try
            {
                ...
                {
                    serchadata.List = serchadata.List.Where(x => x.Isdelete == false
                                         && x.ExpenseDate >= Convert.ToDateTime(monthStart) && x.ExpenseDate <= Convert.ToDateTime(monthEnd)).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public IActionResult ExcelExport(string sdate, string edate)
        {
            var todaynow = DateTime.Today;
            var monthStart = new DateTime(todaynow.Year, todaynow.Month, 1);
            var monthEnd = monthStart.AddMonths(1).AddDays(-1);
            ...
            var data = (from expense in _context.ExpenseMaster
                        where expense.Isdelete == false &&
                        expense.ExpenseDate >= (sdate != null && sdate != "" ? Convert.ToDateTime(sdate) : Convert.ToDateTime(monthStart))
                        && expense.ExpenseDate <= (edate != null && edate != "" ? Convert.ToDateTime(edate) : Convert.ToDateTime(monthEnd))
                        orderby expense.Id descending
                        select new ExpenseMasterViewModel
                        {
                            Id = expense.Id,
            ...
            var fileName = string.Format("Expense Data-{0}.xls", DateTime.Now.ToString("dd.MM.yyyy"));

            using (var workbook = new XLWorkbook())
            {
                IXLWorksheet worksheet =
                workbook.Worksheets.Add("Expense Data");
                worksheet.Cell(1, 1).Value = "Sr No.";
                worksheet.Cell(1, 2).Value = "Name";
                worksheet.Cell(1, 3).Value = "Exp Type";
                ...
                for (int index = 1; index <= data.Count(); index++)
                {
                    ...
                    worksheet.Cell(index + 1, 2).Value = data[index - 1].ExpName;
                    worksheet.Cell(index + 1, 3).Value = data[index - 1].exptype;
                    worksheet.Cell(index + 1, 4).Value = data[index - 1].Amount;
                    worksheet.Cell(index + 1, 5).Value = data[index - 1].ExpenseDate;
                    worksheet.Cell(index + 1, 6).Value = data[index - 1].Description;
                    worksheet.Cell(index + 1, 7).Value = data[index - 1].CreatedDate;
                    worksheet.Cell(index + 1, 8).Value = data[index - 1].CreatedBy;
                }
